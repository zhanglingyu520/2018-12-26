...
app.listen(CONFIG.PORT,()=>{
    console.log(`THE WEB SERVICE IS CREATED SUCESSFULLY AND IS LINSTENING TO THE PORT :${CONFIG.PORT}`);
});
/*-CREATE WARE-*/
//=>实现CROS可以的中间件
...
//=>把所有POST请求主体承担的内容进行解析，把URL-ENCODED格式转化为对象，存放到REQ.BODY属性上的
const bodyParser =require('body-parser');
app.use(bodyParser.urlencoded({extended:false}));
app.use(bodyParser.json());
//=>在所有的请求开始之前，把JSON中的数据获取到，挂载到REQ的某些属性上，以后想要获取，直接从属性读取即可
...
app.use(async(req,res,next)=>{
    ...
    req.storeDAT=JSON.parse(await readFile(STORE_PATH));
